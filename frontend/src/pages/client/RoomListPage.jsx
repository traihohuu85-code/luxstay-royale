  ...
  return (
    <div className="container-lux py-12">
      <div className="glass-panel p-6 md:p-8">
        <div className="mb-7 flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><p className="section-kicker">Danh sách phòng</p><h1 className="mt-3 font-display text-4xl font-bold text-white md:text-5xl">Tìm phòng theo cơ sở, danh mục và thời gian</h1><p className="mt-3 max-w-3xl text-slate-400">Bộ lọc mượt mà kết nối trực tiếp API, hỗ trợ kiểm tra phòng trống theo khoảng ngày.</p></div><button onClick={reset} className="btn-secondary w-fit"><SlidersHorizontal size={16} />Xóa lọc</button></div>
        <div className="grid gap-4 md:grid-cols-6">
          ...
          <label className="relative"><CalendarDays className="pointer-events-none absolute left-4 top-1/2 z-10 -translate-y-1/2 text-amber-200" size={18} /><input className="client-input pl-11" type="date" value={filter.checkInDate || ''} onChange={(e) => update('checkInDate', e.target.value)} /></label>
