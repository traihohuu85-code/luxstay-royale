  ...
  return (
    <div className="container-lux py-12">
      <div className="glass-panel p-6 md:p-8">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-4"><label className="relative"><Search className="pointer-events-none absolute left-4 top-1/2 z-10 -translate-y-1/2 text-amber-200" size={18} /><input className="client-input pl-11" placeholder="Tìm theo tên phòng..." value={filter.keyword || ''} onChange={(e) => update('keyword', e.target.value)} /></label><input className="client-input" type="number" placeholder="Giá từ" value={filter.minPrice || ''} onChange={(e) => update('minPrice', e.target.value)} /><input className="client-input" type="number" placeholder="Giá đến" value={filter.maxPrice || ''} onChange={(e) => update('maxPrice', e.target.value)} /><select className="client-input" value={filter.status || ''} onChange={(e) => update('status', e.target.value)}><option value="">Mọi trạng thái</option><option value="available">Còn phòng</option><option value="occupied">Đang sử dụng</option><option value="maintenance">Bảo trì</option></select></div>
      </div>
